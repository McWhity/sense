...
class Scatterer(object):
    def __init__(self, **kwargs):
        ...
        assert self.sigma_s_hh is not None, (
            'Particle HH scattering cross section needs to be specified [m**-2]'
        )

        self.sigma_s_vv = kwargs.get('sigma_s_vv', None)  # particle scattering cross area
        assert self.sigma_s_vv is not None, (
            'Particle VV scattering cross section needs to be specified [m**-2]'
        )

        self.sigma_s_hv = kwargs.get('sigma_s_hv', None)  # particle scattering cross area
        assert self.sigma_s_hv is not None, (
            'Particle HV scattering cross section needs to be specified [m**-2]'
        )


class ScatIso(Scatterer):
    """Isotropic scatterer definition see 11.2 in Ulaby (2014)."""
    def __init__(self, **kwargs):
...
class ScatRayleigh(Scatterer):
    """Rayleigh scatterer definition see 11.2 in Ulaby (2014)."""
    def __init__(self, **kwargs):
